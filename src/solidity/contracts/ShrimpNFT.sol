...
interface IShrimp is IERC20{ 
 function isLevel(address customerAddr) external returns(uint LevelCount);
}
...
contract ShrimpNFT is ERC1155, Ownable, ERC1155Burnable  {
    ...
    constructor(address _stakingToken) ERC1155("https://game.example/api/item/{id}.json") {
    ...
    function InquireUserNFT (address UserAddress) public view returns (string memory) {
        ...
        if(balanceOf(UserAddress,2) >= 1){
            ...
            UserNFTId = 2;
            return "Copper";
        }else{return "User has no Shrimp NFT";}
